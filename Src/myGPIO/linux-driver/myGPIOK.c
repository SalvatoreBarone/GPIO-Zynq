...
#define myGPIOK_MODE_OFFSET		0x00U	//!< @brief Offset, rispetto all'indirizzo base, del registro "mode" per il device myGPIO
#define myGPIOK_WRITE_OFFSET	0x04U	//!< @brief Offset, rispetto all'indirizzo base, del registro "write" per il device myGPIO
#define myGPIOK_READ_OFFSET		0x08U	//!< @brief Offset, rispetto all'indirizzo base, del registro "read" per il device myGPIO
 ...
 */
static void myGPIOK_GlobalInterruptEnable(void* baseAddress) {
 ...
 * @param [in] baseAddress indirizzo virtuale del device
 */
static void myGPIOK_GlobalInterruptDisable(void* baseAddress) {
 ...
 * vengono abilitati;
 */
static void myGPIOK_PinInterruptEnable(void* baseAddress, unsigned mask) {
 ...
 */
static void myGPIOK_PinInterruptDisable(void* baseAddress, unsigned mask) {
	unsigned reg_value = ioread32((baseAddress + myGPIOK_PIE_OFFSET));
	reg_value &= ~mask;
	iowrite32(reg_value, (baseAddress + myGPIOK_PIE_OFFSET));
}
 ...
 */
static unsigned myGPIOK_PendingPinInterrupt(void* baseAddress) {
 ...
 * vengono notificati;
 */
static void myGPIOK_PinInterruptAck(void* baseAddress, unsigned mask) {
...
typedef struct {
	...
	uint32_t can_read; /**< Flag "puoi leggere"
	 	 	 	 	 	 	Il valore viene settato dalla funzione myGPIOK_irq_handler() al manifestarsi di un
	 	 	 	 	 	 	interrupt, prima di risvegliare i processi in attesa di un interrupt.
							I processi che effettuano read() bloccante restano bloccati finoche' int_occurred = 0
							*/
 ...
 *
 */
	if ((error = cdev_add(&myGPIOK_dev_ptr->cdev, myGPIOK_Mm_number, MAX_NUM_OF_DEVICES)) != 0) {
 ...
 */
	...
	XGpio_Channel_Interrupt(myGPIOK_dev_ptr->vrtl_addr, XGPIO_USED_INT);
#else
