...
/**
 * Se non viene specificato il device myGPIOK col quale interagire e' impossibile continuare.
 ...
 * Ad ogni periferica compatibile con il driver myGPIOK e' associato un file diverso in /dev/ attraverso il
 * quale e' possibile interagire con il device. Rispetto al driver UIO, non e' necessario effettuare nessuna
 * operazione di mapping, in quanto il driver myGPIOK tradurra' le sistem-call che normalmente verrevvero
 ...
 * Se l'apertura del file non riesce e' impossibile continuare.
 * Per questo motivo, in questo caso, la funzione restituisce -1, per cui il programma viene terminato.
 */
 ...
 * <br>
 * Nel caso un cui si usi un driver ad-hoc, e' possibile usare le funzioni read() e write() per interagire
 ...
 * Per impostare la modalita' di funzionamento e' necessario scrivere sul registro MODE. L'offset di tale
 * registro e' determinato in base al particolare device che si sta' utilizzando.
 ...
 * E' possibile, definendo la macro __USE_PWRITE__, usare la funzione pwrite(), che combina le due operazioni.
 */
	if (param->op_mode == 1) {
 ...
 * Per impostare il valore dei pin del device e' necessario scrivere sul registro WRITE. L'offset di tale
 * registro e' determinato in base al particolare device che si sta' utilizzando.
 ...
 * E' possibile, definendo la macro __USE_PREAD__, usare la funzione pread(), che combina le operazioni di
 * seek() e read().
